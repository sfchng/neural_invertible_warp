...
class Pose:
    ...
    def save_subset_projection_matrix_for_dtu(self, opt, poses_w2c, mode="init"):
        ...
        print("saved projection matrix for dtu....")
...
class Model(nerf_inn_dtu.Model, Pose):
    ...
    def build_pose_net(self, opt):
        ...
        assert (opt.pose.parameterization == "inn")

        self.pose_net = INNPoseParams(opt, num_poses=len(self.train_data), 
    ...
    def setup_optimizer(self,opt):
        ...
        self.optim_pose = optimizer([dict(params=self.pose_net.pose_embedding.parameters(),lr=opt.optim.lr_pose)])
        self.optim_pose.add_param_group(dict(params=self.pose_net.pose_latent.parameters(), lr=opt.optim.lr_pose))
    ...
    def get_all_training_poses(self,opt):
        ...
        ## NOTE::: treat this as pose corrections,     
        pose_pred_refine_w2c = self.pose_net.get_w2c_poses()
        pose_pred_w2c = camera.pose.compose([pose_pred_refine_w2c, self.pose_net.initial_poses_w2c])
    ...
    def evaluate_full(self,opt):
        ...
        ## ##   
        self.save_subset_projection_matrix_for_dtu(opt, pose_aligned, mode="pred_inn")

        error = self.evaluate_camera_alignment(opt,pose_aligned,pose_GT)
        log.info("--------------------------")
        log.info("rot:   {:8.3f}".format(np.rad2deg(error.R.mean().cpu())))
        log.info("trans: {:10.5f}".format(error.t.mean()))
        log.info("--------------------------")
        # dump numbers
...
class Graph(nerf_inn_dtu.Graph):
    ...
    def get_pose(self,opt,var,mode=None,iter=None):
        return self.get_w2c_pose(opt, var, mode, iter)
    
    def get_w2c_pose(self,opt,var,mode=None, iter=None):
        if mode=="train":
            ## get the current estimates of the camera poses, which are optimized
            # For INN, it will be the warped ray
            #
            assert iter is not None, "ERROR: Iteration is needed for the c2f embedding in INN"
            ray, center_3D, grid_3D  = self.pose_net.get_warped_rays_in_world(var, mode, iter) 
            return ray, center_3D, grid_3D
