...
class Model(base.Model):
    ...
    def evaluate_full(self,opt,eps=1e-10):
        ...
        for i,batch in enumerate(loader):
            ...
            if opt.model in ["barf","barf_se3_field"] and opt.optim.test_photo:
...
class Graph(base.Graph):
    ...
    def forward(self,opt,var,mode=None):
        ...
        if opt.nerf.rand_rays and mode in ["train","test-optim"]:
            # start_time = time.time()           
            pose = self.get_pose(opt,var,mode=mode) #(:,3,4)            
            ...
            pose = self.get_pose(opt,var,mode=mode, ind=random_ind) #(:,3,4)
            ret = self.render_by_slices(opt,pose[random_ind][None],intr=var.intr[random_ind][None],mode=mode)
...
class NeRF(torch.nn.Module):
    ...
    def define_network(self,opt):
        ...
        for li,(k_in,k_out) in enumerate(L):
            ...
            self.total_param += linear.weight.numel()
        # RGB prediction
        ...
        log.info("Total number of params {}".format(self.total_param))
