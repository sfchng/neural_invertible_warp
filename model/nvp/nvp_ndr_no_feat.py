...
class DeformNetwork(nn.Module):
    def __init__(self,
        ...
        if actfn == "silu":
            print ("WARNING:: INN using silu function")
            self.activation = nn.SiLU()
        elif actfn == "relu":
            print ("WARNING:: INN using ReLU")
            self.activation = nn.ReLU()
        elif actfn == "sine":
            print ("WARNING:: INN using Sine")
            self.activation = SirenLayer()
        elif actfn == "gaussian":
            print ("WARNING::  INN using Gaussian function")
            self.activation = GaussianLayer()
        else:
            self.activation = nn.Softplus(beta=100)


    def forward(self, deformation_code, input_pts, alpha_ratio=0):
        """
        Args:
            deformation_code (N,3)
            input_pts (N,3)
            alpha_ratio: annealing_ratio (revisit later)
        """
        batch_size = input_pts.shape[0]
        x = input_pts
        for i_b in range(self.n_blocks):
            form = (i_b // 3) % 2
            mode = i_b % 3  # to determine which to split ##
            #print("form {} mode {}".format(form, mode))
            ...
            ## expand features ##
            ## add a function ##
            #deform_code_ib = self._expand_features(deform_code_ib, x)  #(B,N,1,C)
            ...
            if form == 0:
                # zyx
                if mode == 0:
                    x_focus = x[..., [2]]  # focus on z
                    x_other = x[..., [0,1]] # x,y
                elif mode == 1:
                    x_focus = x[..., [1]]  # focus on y
                    x_other = x[..., [0,2]] # x,z
                else:
                    x_focus = x[..., [0]]  # focus on x
                    x_other = x[..., [1,2]] # y,z
            else:
                # xyz
                if mode == 0:
            ...
            ## Decompose Ruv and delta_uv ##
            R_uv = x[...,[0]]
            delta_uv = x[...,1:]
            rot_2d = euler2rot_2dinv(R_uv.view(-1,1))
            trans_2d = delta_uv.view(-1,2)
            
            #rot_2d = euler2rot_2dinv(x[..., [0]])
            #trans_2d = x[..., 1:]
            x_other = torch.bmm(rot_2d, (x_ori.view(-1,2) - trans_2d)[...,None]).squeeze(-1)
            x_other = x_other.reshape(x_ori.shape[0], x_ori.shape[1], 1, 2)
            if form == 0:
                if mode == 0:
    ...
    def inverse(self, deformation_code, input_pts, alpha_ratio):
        ...
        Leave it for now as inverse function will not be used...
        """
        batch_size = input_pts.shape[0]
        x = input_pts
        for i_b in range(self.n_blocks):
            ...
            rot_2d = euler2rot_2d(x[..., [0]].view(-1,1))  #(B,2,2)
            trans_2d = x[..., 1:].view(-1,2)
            x_focus = torch.bmm(rot_2d, x_focus.view(-1,2)[...,None]).squeeze(-1) + trans_2d
            x_focus = x_focus.reshape(x_ori.shape[0], x_ori.shape[1], 1, 2)
            # part a
    ...
    def _expand_features(self, feature, x):
        _, N, K, _ = x.shape
        return feature[:,None,None,:].expand(-1, N, K,-1)
...
class SirenLayer(nn.Module):
    def __init__(self, w=10, trainable=False):
        super().__init__()
...
if __name__ =="__main__":
    localizeInn = DeformNetwork(d_feature=128, d_in=3, d_out_1=1, d_out_2=3, n_blocks=3, d_hidden=128, n_layers=1,
                skip_in=[], multires=6, weight_norm=True)
    
    optim_pose = optimizer([dict(params=localizeInn.parameters(),lr=5.e-4)])