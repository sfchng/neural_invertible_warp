...
class Model(nerf_gaussian.Model):
    ...
    def get_all_training_poses(self,opt):
        ...
        embedding = self.graph.warp_embedding.weight #(B,C)
        se3_refine = self.graph.warp_mlp(opt, embedding)
...
class Graph(nerf_gaussian.Graph):
    ...
    def get_pose(self,opt,var,mode=None):
        if mode=="train":
            ...
            ## forward warp_mlp ##
            ## get camera coordinates ##
            embedding = self.warp_embedding.weight[var.idx] #(B,C)
            var.se3_refine = self.warp_mlp(opt, embedding)
...
class localWarp(torch.nn.Module):
    def __init__(self, opt):
        ...
        for li,(k_in,k_out) in enumerate(L):
            ...
            if li == len(L):
                torch.nn.init.uniform_(linear.weight, -1.e-5, 1.e-5)
                torch.nn.init.zeros_(linear.bias)
            self.mlp_warp.append(linear)
        

    def gaussian(self, opt, feat):
        out = (-0.5*(feat)**2/opt.arch.sigma_warp**2).exp()
        return out
    ...
    def forward(self,opt,uvf):
        ...
        for li,layer in enumerate(self.mlp_warp):
            ...
            if li!=len(self.mlp_warp)-1:
                if opt.arch.actfn_warp == "gaussian":
