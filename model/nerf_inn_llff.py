...
class Model(base.Model):
    ...
    #             if self.opt.weight_sched[key].start_decay is not None:
    #                 ## add weight scheduler ##
    #                 self.weight_sched(key)
    #     loss.update(all=loss_all)
    #     return loss
    
    def weight_sched(self, key):
        if self.it >= self.opt.weight_sched[key].start_decay:
            self.opt.loss_weight[key] +=1
    ...
    def log_scalars(self,opt,var,loss,metric=None,step=0,split="train"):
        ...
        if split=="train":
            ...
            if opt.model in ["barf_inn_nvp", "barf_inn_nvp_v2"]:
                self.tb.add_scalar("{0}/{1}".format("c2f","inn"),var.inn_posenc_alpha,step)
                
            if opt.model in ["barf_inn_nvp", "barf_inn_nvp_v2"] and opt.barf_c2f is not None:
                self.tb.add_scalar("{0}/{1}".format("c2f","nerf"),var.nerf_posenc_alpha,step)
    ...
    def evaluate_full(self,opt,eps=1e-10):
        ...
        for i,batch in enumerate(loader):
            ...
            if opt.model in ["barf_inn_v2", "barf_inn_nvp","barf_inn_nvp_v2"] and opt.optim.test_photo:
    ...
    def generate_videos_synthesis(self,opt,eps=1e-10):
        ...
        if opt.data.dataset=="blender":
            ...
            poses = pose_pred if opt.model in ["barf_inn", "barf_inn_v2", "barf_inn_nvp", "barf_inn_nvp_v2"]  else pose_GT
            if opt.model in ["barf_inn", "barf_inn_v2", "barf_inn_nvp", "barf_inn_nvp_v2"] and opt.data.dataset in ["llff", "llff_modified"]:
    ...
    def verify_se3(self, opt):
        ...
        ## get the ray in the camera coordinate frame ##
        center_cam, grid_cam = camera.get_unwarped_center_and_ray(opt,intr=var.intr,ray_idx=var.ray_idx) #(B,N,3),(B,N,3)

   
        ## create an identity pose ##
        pose_init = torch.eye(3, 4).to(opt.device)
        pose_init = pose_init[None].repeat(len(var.idx),1,1 )

        center_cam = center_cam.detach()
        grid_cam = grid_cam.detach()    

        feat = self.graph.warp_latent.weight
        camera_coords_3D = torch.cat([grid_cam, center_cam], axis=1)  #(B,N+N,3)       
        
        basis_origin = np.array([0,0,0])
        basis_origin = torch.from_numpy(basis_origin)[None][None].to(opt.device)        
        
        
        #basis_vec1 = np.array([1,0,0])
        #basis_vec2 = np.array([0,1,0])
        for i in range(10):
            basis_vec1 = torch.randn(3)
            basis_vec2 = torch.randn(3)
            

            basis_vec1 = (basis_vec1)[None][None].to(opt.device)
            basis_vec2 = (basis_vec2)[None][None].to(opt.device)
            
            camera_coords_3D_warped = self.graph.warp_mlp.forward(feat, camera_coords_3D.unsqueeze(2))
            
            b = self.graph.warp_mlp.forward(feat[0][None], basis_origin.unsqueeze(2)) 
            
            basis_vec1_warped = self.graph.warp_mlp.forward(feat[0][None], basis_vec1.unsqueeze(2))
            basis_vec2_warped = self.graph.warp_mlp.forward(feat[0][None], basis_vec2.unsqueeze(2))         

            #grid_3D = camera_coords_3D_warped[:,:len(var.ray_idx),:]  #(B,N,1,3)
            #center_3D = camera_coords_3D_warped[:,len(var.ray_idx):,:]  #(B,N,1,3)

            # ## take one frame ##
            #grid_3D_first_frame = grid_3D[30].squeeze(2)
            #grid_cam_first_frame = grid_cam[30]

            #grid_3D_first_frame_2 = grid_3D_first_frame - basis_vec1_warped.squeeze(0)
            
            basis_vec1_warped_b = basis_vec1_warped - b
            basis_vec2_warped_b = basis_vec2_warped - b
            basis_vec1_warped_b = basis_vec1_warped_b.squeeze(0).squeeze(0).squeeze(0)
            basis_vec2_warped_b = basis_vec2_warped_b.squeeze(0).squeeze(0).squeeze(0)
            # ## take pairwise pair ##
            
            print ("-------------------------------------------------")                
            angle_degree_1 = self.compute_angle_between_two_vectors(basis_vec1_warped_b.detach().cpu().numpy(), basis_vec2_warped_b.detach().cpu().numpy())
            angle_degree_2 = self.compute_angle_between_two_vectors(basis_vec1.squeeze(0).squeeze(0).detach().cpu().numpy(), basis_vec2.squeeze(0).squeeze(0).detach().cpu().numpy())
            print("angle before warp :::: {}".format(angle_degree_1))
            print("angle after warp :::: {}".format(angle_degree_2))
            
            ## computing the norm #W#
            print("(vec1) norm before warp :::: {}".format(np.linalg.norm(basis_vec1.detach().cpu().numpy())))
            print("(vec1) norm after warp :::: {}".format(np.linalg.norm(basis_vec1_warped.detach().cpu().numpy())))
            
            print("(vec2) norm before warp :::: {}".format(np.linalg.norm(basis_vec2.detach().cpu().numpy())))
            print("(vec2) norm after warp :::: {}".format(np.linalg.norm(basis_vec2_warped.detach().cpu().numpy())))
            print ("-------------------------------------------------")
            
        print('here')

    @torch.no_grad()
    def verify_se3_orig(self, opt):
        ...
        basis_vec1 = np.array([0,0,0])
        basis_vec2 = np.array([0,1,0])
        basis_vec1 = torch.from_numpy(basis_vec1)[None][None].to(opt.device)
        basis_vec2 = torch.from_numpy(basis_vec2)[None][None].to(opt.device)
        

         
        camera_coords_3D_warped = self.graph.warp_mlp.forward(feat, camera_coords_3D.unsqueeze(2))
        basis_vec1_warped = self.graph.warp_mlp.forward(feat[30][None], basis_vec1.unsqueeze(2)) 
        #basis_vec2_warped = self.graph.warp_mlp.forward(feat, basis_vec2.unsqueeze(2))         
        ...
        # compute dot product ##
        angle_degrees_world = []
        for i in range(grid_3D_first_frame_2.shape[0]-2):
            ## take two vectors ##
            vec1 = grid_3D_first_frame_2[i].detach().cpu().numpy().squeeze(0)
            vec2 = grid_3D_first_frame_2[i+2].detach().cpu().numpy().squeeze(0)
            angle_degrees_world.append(self.compute_angle_between_two_vectors(vec1, vec2))
        
        angle_degrees_cam = []
        for j in range(grid_cam_first_frame.shape[0]-2):
            ## take two vectors ##
            vec1 = grid_cam_first_frame[j].detach().cpu().numpy()
            vec2 = grid_cam_first_frame[j+2].detach().cpu().numpy()
            angle_degrees_cam.append(self.compute_angle_between_two_vectors(vec1, vec2))
            
        #angle_degree_1 = self.compute_angle_between_two_vectors(basis_vec1.detach().cpu().numpy().squeeze(0).squeeze(0), basis_vec2.detach().cpu().numpy().squeeze(0).squeeze(0))
        #angle_degree_2 = self.compute_angle_between_two_vectors(basis_vec1_warped.detach().cpu().numpy().squeeze(0).squeeze(0).squeeze(0), basis_vec2_warped.detach().cpu().numpy().squeeze(0).squeeze(0).squeeze(0))
        print(angle_degrees_world)
        print(angle_degrees_cam)
        print('here')
    ...
    def debug_inn(self, opt):
        ...
        ## get groundtruth camera center and ray ## 
        center_cam_gt, ray_gt = camera.get_center_and_ray(opt,var.pose,var.intr)
        
        ## use camera pose to transform the camera center from frame 1 to frame 2 ##
        cam1_center_gt = center_cam_gt[0,0]  # w2_c1
        cam2_center_gt = center_cam_gt[1,0]  # w2_c2
        
        ## pose is an extrinsic matrix which w2c ##
        cam1_c2 = camera.world2cam(cam1_center_gt, var.pose[1])
        cam1_c1 = camera.world2cam(cam1_center_gt, var.pose[0])
        print ('here')
        ...
        ## prealign camera ##
        center_GT = center_cam_gt[:,0,:] #(N,3)
        center_pred = center_3D[:,0,:].squeeze(1) #(N,3)
        sim3 = camera.procrustes_analysis(center_GT,center_pred)
        center_aligned = (center_pred-sim3.t1)/sim3.s1@sim3.R.t()*sim3.s0+sim3.t0

        ## assuming you have the camera groundtruth ##
        ## how do you obtain the center of camera 2 in its frame ##
        

        ## given camera camera center in frame 1, query where the camera center is in frame 2 ## (0->2)
        camera_coords_3D_2 = self.graph.warp_mlp.inverse(feat[1,:][None], camera_coords_3D_warped[0][None], alpha_ratio=1)

        ## take the global transformation ##

        ## warp the camera center ##
        camera_coords_3D_out = self.graph.warp_mlp.inverse(feat, camera_coords_3D_warped, alpha_ratio=1) ##(observation-to-world)
...
class Graph(base.Graph):
    ...
    def forward(self,opt,var,mode=None,iter=None):
        ## following l2g, rescale the size of the scene condition on the pose ##
        if opt.data.dataset == "blender":
            if opt.camera.noise_type == "l2g":
                depth_min,depth_max = opt.nerf.depth.range
                position = camera.Pose().invert(self.global_rigid.weight.data.detach().clone().view(-1,3,4))[...,-1]
        ...
        if opt.nerf.rand_rays and mode in ["train","test-optim"]:
            ...
            #v0: augmented get_pose fn ##
            if mode == "train":

                pose_init = self.get_pose_init(opt,var,mode=mode,iter=iter)

                ## get camera center and camera grid, which will be used for 
                ## global transformation
                center_cam, grid_cam = camera.get_unwarped_center_and_ray(opt,intr=var.intr,ray_idx=var.ray_idx, pose_init=pose_init) #(B,N,3),(B,N,3)

                ray, center, grid_3D, alpha_ratio = self.get_pose(opt, var, mode=mode, iter=iter) #(B,N,3), (B,N,3), (B,N,3)

                ##v0: remove ray_idx as it will be handled in get_pose function ##
                ret = self.render_local(opt,ray,center,intr=var.intr,mode=mode) # [B,N,3],[B,N,1]

                ret.update(grid_3D=grid_3D, center=center, grid_cam=grid_cam, center_cam=center_cam, inn_posenc_alpha=alpha_ratio)
            elif mode == "test-optim":
                pose = self.get_pose(opt,var, mode=mode)
                ret = self.render(opt, pose, intr=var.intr, ray_idx=var.ray_idx, mode=mode)

        elif mode == "render_train":
            random_ind = np.random.choice(len(var.idx))
            #v0: augmented get_pose fn ##
            ray, center = self.get_pose(opt, var, mode=mode,ind=random_ind)
            # render full image (process in slices)
            ret = self.render_by_slices_local(opt,ray,center,intr=var.intr[random_ind][None],mode=mode) if opt.nerf.rand_rays else \
                  self.render_local(opt,ray, center,intr=var.intr,mode=mode) # [B,HW,3],[B,HW,1]
            var.render_train_idx = random_ind
        else:
            ## render full images ##
            pose = self.get_pose(opt, var, mode=mode)
    ...
    def compute_loss(self,opt,var,mode=None):
        ...
        ## project the per-point transformation to be constrained by 
        ## the global transformation
        if opt.loss_weight.global_alignment is not None and mode=="train":
            
            ## predicted local coordinates ##
            source = torch.cat([var.grid_cam, var.center_cam], dim=1)  #(B,2N,3)
            ...
            self.global_rigid.weight.data = svd_poses.detach().clone().view(-1,12)
    ...
    def render_local(self,opt,ray,center,intr=None,ray_idx=None,mode=None):
        ...
        ## v0: Does not matter for now; just leave it as it is
        if opt.camera.ndc:
        ...
        ret = edict(rgb=rgb,depth=depth,opacity=opacity) 
    ...
    def render_by_slices_local(self,opt,ray, center,intr=None,mode=None):
        ...
        for c in range(0,opt.H*opt.W,opt.nerf.rand_rays):
            ...
            ret = self.render_local(opt,ray, center,intr=intr,ray_idx=ray_idx,mode=mode) # [B,R,3],[B,R,1]
    ...
    def render(self,opt,pose,intr=None,ray_idx=None,mode=None):
        ...
        # render with main MLP
        depth_samples = self.sample_depth(opt,batch_size,num_rays=ray.shape[1]) # [B,HW,N,1]
        if 'barf'.lower() in opt.model.lower() and opt.barf_c2f is not None:
            rgb_samples,density_samples, _ = self.nerf.forward_samples(opt,center,ray,depth_samples,mode=mode)
        else:
            rgb_samples,density_samples = self.nerf.forward_samples(opt,center,ray,depth_samples,mode=mode)
        rgb,depth,opacity,prob = self.nerf.composite(opt,ray,rgb_samples,density_samples,depth_samples)
