...
class Stiefel():
    """
    Utility to convert for Stiefel manifold 
    https://openaccess.thecvf.com/content_CVPR_2019/papers/
    Zhou_On_the_Continuity_of_Rotation_Representations_in_Neural_Networks_CVPR_2019_paper.pdf
    """

    def orthogonal_6d_to_ST(self, w):
        """
        Args:
            w (torch.Tensor [B,9])
        ...
        y = self.normalize(y_raw - self.proj_u2a(x, y_raw))
        z = self.cross_product(x, y)
    ...
    def normalize(self, r):
        ...
        Args:
            r (torch.Tensor [B,3])
        """
        r_mag = torch.sqrt((r**2).sum(1, keepdim=True))
        r_mag = torch.clamp(r_mag, min=1.e-8)
        r = r / r_mag
        return r
    
    def proj_u2a(self, u, a):
        """
        Args:
            u (torch.Tensor [B,3])
            a (torch.Tensor [B,3])
...
def get_unwarped_center_and_ray(opt, intr=None, ray_idx=None, pose_init=None): #[HW,2]
    """
    Given the intrinsic matrix, get the camera center and grid coordinates in camera coordinates

    Return:
        center_3D (torch.Tensor[B,HW,3])
        grid_3D (torch.Tensor[B,HW,3])
    """
    ...
    if pose_init is not None:
        grid_3D = cam2world(grid_3D,pose_init) # [B,HW,3]
        center_3D = cam2world(center_3D,pose_init) # [B,HW,3]

    if ray_idx is not None:
        ## consider only subset of rays
        center_3D, grid_3D = center_3D[:,ray_idx], grid_3D[:,ray_idx]
    
    return center_3D,grid_3D   


def get_center_and_ray_camera_and_transform(opt,pose,intr=None): # [HW,2]
    """
    Note to myself:
    Warning do not use: It's for self-learning 
    """
    # given the intrinsic/extrinsic matrices, get the camera center and ray directions]
    assert(opt.camera.model=="perspective")
    with torch.no_grad():
    ...
    #### transform from camera to world coordinates ####
    pose_inv = Pose().invert(pose) ## inverse because pose is an extrinsic matrix (world->cam)
    ray_world = ray @ pose_inv[...,:3].transpose(-1,-2)
    center_3D = cam2world(center_3D,pose) # [B,HW,3]
    return center_3D,ray_world

def get_center_and_ray(opt,pose,intr=None): # [HW,2]
    """
    Note to myself:
    the origin of all rays (a.k.a center_3D) is the camera origin in world coordinate 

    grid_3D, center_3D is defined in camera coordinates before the cam2world() transformation
    """
    # given the intrinsic/extrinsic matrices, get the camera center and ray directions]
    assert(opt.camera.model=="perspective")
    with torch.no_grad():
...
def get_center_and_ray_split(opt,pose,intr=None, ray_idx=None): # [HW,2]
    """
    This function is created to solve memory issue 
    to test the idea of using per-point se(3)
    """
    # given the intrinsic/extrinsic matrices, get the camera center and ray directions]
    assert(opt.camera.model=="perspective")
    with torch.no_grad():
    ...
    ## tedious fix, let's just ray as it is, and batch_size=number_of_rays
    ## NOTE: Revisit later, may cause bug ##
    batch_size = 1
    xy_grid = xy_grid.unsqueeze(1) # [B,HW,2]
    grid_3D = img2cam(to_hom(xy_grid),intr) # [B,HW,3]
    center_3D = torch.zeros_like(grid_3D) # [B,HW,3]
    # transform from camera to world coordinates
    grid_3D = cam2world(grid_3D,pose) # [B,HW,3]
...
def gather_camera_cords_grid_3D(opt,batch_size,intr=None,ray_idx=None): # [HW,2]
    ...
    with torch.no_grad():
        ...
        # compute grid_3D
        ## NOTE:::: Because the batch_size is now the HW, so 
        ## we do not have to do anything for localization 
        xy_grid = xy_grid.repeat(batch_size,1,1) # [B,HW,2]
        grid_3D = img2cam(to_hom(xy_grid),intr) # [B,HW,3]
        if ray_idx is not None:
            ## consider only a subset of rays ##
            grid_3D = grid_3D[:,ray_idx]
    return grid_3D

def gather_camera_cords_grid_3D_camera(opt,batch_size,intr=None,ray_idx=None, pose=None): # [HW,2]
    # given the intrinsic matrices, get the grid_3D]
    assert(opt.camera.model=="perspective")
    with torch.no_grad():
        # compute image coordinate grid
        ...
        xy_grid = xy_grid.repeat(batch_size,1,1) # [B,HW,2]
        grid_3D = img2cam(to_hom(xy_grid),intr) # [B,HW,3]
        grid_3D = cam2world(grid_3D, pose)
        if ray_idx is not None:
            ## consider only a subset of rays ##
            grid_3D = grid_3D[:,ray_idx]
    return grid_3D
