...
def apply_homography(opt, xy_grid, warp_matrix):
    """
    Args:
        opt
        xy: source points
        warp_matrix: homography matrix (torch.Tensor [B,3,3])
    """
    warp_matrix = warp_matrix.to(opt.device)
...
def get_normalized_pixel_corners_crop(opt):
    y_crop = (opt.H//2-opt.H_crop//2,opt.H//2+opt.H_crop//2)
    x_crop = (opt.W//2-opt.W_crop//2,opt.W//2+opt.W_crop//2)
    ...
    corners = torch.tensor(corners,dtype=torch.float32,device=opt.device).repeat(opt.batch_size,1,1)  #(B,4.2)   
    return corners

def get_normalized_pixel_corners_crop_v2(opt, random=0):

    y_crop = (opt.H//2-opt.H_crop//2 + random,opt.H//2+opt.H_crop//2 - random)
    x_crop = (opt.W//2-opt.W_crop//2 + random,opt.W//2+opt.W_crop//2 - random)
    Y = [((y+0.5)/opt.H*2-1)*(opt.H/max(opt.H,opt.W)) for y in y_crop]
    X = [((x+0.5)/opt.W*2-1)*(opt.W/max(opt.H,opt.W)) for x in x_crop]
    corners = [(X[0],Y[0]),(X[0],Y[1]),(X[1],Y[1]),(X[1],Y[0])]   
    corners = torch.tensor(corners,dtype=torch.float32,device=opt.device).repeat(opt.batch_size,1,1)  #(B,4.2)   
...
def warp_corners_use_homographies(opt, warp_matrix1, warp_matrix2):
    y_crop = (opt.H//2-opt.H_crop//2,opt.H//2+opt.H_crop//2)
    x_crop = (opt.W//2-opt.W_crop//2,opt.W//2+opt.W_crop//2)
    Y = [((y+0.5)/opt.H*2-1)*(opt.H/max(opt.H,opt.W)) for y in y_crop]
    X = [((x+0.5)/opt.W*2-1)*(opt.W/max(opt.H,opt.W)) for x in x_crop]
    corners = [(X[0],Y[0]),(X[0],Y[1]),(X[1],Y[1]),(X[1],Y[0])]
    corners = torch.tensor(corners,dtype=torch.float32,device=opt.device).repeat(opt.batch_size,1,1)
    corners_warped = warp_grid_use_matrix(opt, corners, warp_matrix2)
    corners_warped = warp_grid_use_matrix(opt, corners_warped, warp_matrix1)
    return corners_warped
